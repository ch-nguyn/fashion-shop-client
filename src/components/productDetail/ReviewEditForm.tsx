...
export interface IReviewEditFormProps {
  product: IProduct;
...
export default function ReviewEditForm(props: IReviewEditFormProps) {
  ...
  const onSubmit = (data: any) => {
    setIsLoading(true);
    reviewApi
      .updateReview(props.review._id, {
        rating: rating,
        title: data.title,
        comment: data.comment,
      })
      .then(() => {
        Swal.fire("Updated", "Your review has been updated!", "success").then(
          () => {
      ...
      .catch((e) => Swal.fire("Oops...!", e.response.data.message, "error"))
      .finally(() => setIsLoading(false));
  };

  useEffect(() => {
    setValue("title", props.review.title);
    setValue("comment", props.review.comment);
